package cn.elevendev.swiperefresh.manager;
...
public class VRefreshLayoutManager extends SwipeRefreshLayout.LayoutManager {
    ...
    public int getOrientation() {
        return SwipeRefreshLayout.LayoutManager.VERTICAL;
    }
    ...
    public void measureHeader(
        ...
        final SwipeRefreshLayout.LayoutParams lp =
                (SwipeRefreshLayout.LayoutParams) header.getView().getLayoutParams();
        ...
        if (header.getStyle() == IRefreshView.STYLE_DEFAULT
            ...
            if (height <= 0) {
                if (height == SwipeRefreshLayout.LayoutParams.MATCH_PARENT) {
                    lp.height = SwipeRefreshLayout.LayoutParams.MATCH_PARENT;
                }
            } else {
                lp.height = height;
            }
            ...
            if (height <= 0 && height != SwipeRefreshLayout.LayoutParams.MATCH_PARENT) {
    ...
    public void measureFooter(
        ...
        final IIndicator indicator = mLayout.getIndicator();
        final SwipeRefreshLayout.LayoutParams lp =
                (SwipeRefreshLayout.LayoutParams) footer.getView().getLayoutParams();
    ...
    public void layoutHeaderView(@NonNull IRefreshView<IIndicator> header) {
        ...
        if (mLayout.isDisabledRefresh() || child.getMeasuredHeight() == 0) {
            ...
            if (SwipeRefreshLayout.sDebug) {
                Log.d(TAG, String.format("onLayout(): header: %s %s %s %s", 0, 0, 0, 0));
            }
            return;
        }
        final SwipeRefreshLayout.LayoutParams lp =
                (SwipeRefreshLayout.LayoutParams) child.getLayoutParams();
        final IIndicator indicator = mLayout.getIndicator();
    ...
    public void layoutContentView(@NonNull View content) {
        final SwipeRefreshLayout.LayoutParams lp =
                (SwipeRefreshLayout.LayoutParams) content.getLayoutParams();
    ...
    public void layoutStickyHeaderView(@NonNull View stickyHeader) {
        final SwipeRefreshLayout.LayoutParams lp =
                (SwipeRefreshLayout.LayoutParams) stickyHeader.getLayoutParams();
        final int left = mLayout.getPaddingLeft() + lp.leftMargin;
    ...
    public void layoutStickyFooterView(@NonNull View stickyFooter) {
        final SwipeRefreshLayout.LayoutParams lp =
                (SwipeRefreshLayout.LayoutParams) stickyFooter.getLayoutParams();
        final int left = mLayout.getPaddingLeft() + lp.leftMargin;
